...
def get_credentials():
  ...
  if not pickled_creds:
    ...
    return pickle.load(pickled_creds)
...
class DoAuthHandler(webapp2.RequestHandler):
  ...
  def get(self):
    ...
    flow = pickle.load(pickled_flow)

    # Store the credentials for later use.
    credentials = flow.step2_exchange(code)
    current_user = users.get_current_user().email() 
    memcache_key = _MEMCACHE_CRED_PREFIX + current_user
    memcache.set(memcache_key, pickle.dump(credentials))
  ...
  def post(self):
    ...
    client_secret = self.request.get_all('client_secret')
    # TODO verify the app path is a valid URL to redirect to
    app_path = self.request.get_all('app_path')
    ...
    redirect_uri = '%s/%s/%s' % (app_path, utils.config.BASE_PATH, self.SUFFIX)
    ...
    memcache.set(memcache_key, pickle.dump(flow))
